<template>
  <div id="fff">
    ...
    <h2 :title="abc" @click="abc += '|';isRED = !isRED;curColor = curColor === 'pink' ? 'blue' : 'pink'" :class="{ red: isRED}" class="overflow" :style="{color: curColor,overflow: 'hidden'}">HELLO FK</h2>
    {{isRED}}
  </div>
  <button @click="count++">button</button>
  <button @click="reverse">reverse</button>
  <button @click="title += '!'">append!</button>
  <a href="https://www.niutron.com" @click.prevent>link prevent</a>
  <h2>Checkbox</h2>
  ...
  <h4>form4 {{ form4 }}</h4>
  <ol>
    <TodoItem v-for="item in groceryList" :todo="item" :key="item.id" :id='`part${item.id}`' :class='`part${item.id}`'></TodoItem>
  </ol>
...
<div>
<nuxt-img src="/test.jpg" loading="lazy" quality="80"/>
</div>
<h2>{{ $device }}</h2>
</template>
<script setup>
